...
export default function StaffManagement() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (result.ok) setRows(result.data.filter((u) => u.role === "staff"));
  ...
  const refreshUsers = async () => {
    ...
    if (res.ok) setRows(res.data.filter((u) => u.role === "staff"));
  };
  ...
  return (
    <div className="p-6">
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-800">Staffs</h1>
        <button
